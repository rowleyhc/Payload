...
public:
    ...
    double Kp = 1.5; //Proportional error constant -- MANUAL INPUT
    double Kd = 1.5; //Derivative error constant -- MANUAL INPUT
    int pwmMax=190;
    float pwmScale=1.3;
...
void TADPOLState::runFan(double pwm, int forwardFanPin, int backwardFanPin, int enableFanPin){
  if (pwm > 0){
    if (previousPwm < 0){
      ...
      analogWrite(enableFanPin, int(pwmMax));
      delay(pwmChangeSignDelay);
    }
    digitalWrite(forwardFanPin, HIGH);
    digitalWrite(backwardFanPin, LOW);
...
double TADPOLState::findPWM(double goal, double timeSinceLastIteration){
  ...
  if(!isnan(yaw)){
    if (yaw*yawAvg>0){yawAvg=yawAvg*0.5+yaw*0.5;}
    else if(yaw>90){yawAvg=(360+yawAvg)*0.5+yaw*0.5;}
    else if(yaw<-90){yawAvg=(yawAvg-360)*0.5+yaw*0.5;}
    else{yawAvg=yawAvg*0.5+yaw*0.5;}
  }
